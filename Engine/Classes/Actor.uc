...
var repnotify const ETeam m_eTeam;
var repnotify const ETeam m_eTeamVisibilityFlags;
var bool m_bVisible;
var bool m_bInFOW;
var bool m_bReplicateHidden;
...
replication
{
	...
	if ( bNetOwner && (!bSkipActorPropertyReplication || bNetInitial) && (Role==ROLE_Authority) && bNetDirty )
		Owner, m_eTeam, m_eTeamVisibilityFlags;
}
...
event SetVisible(bool bVisible);

native simulated function bool IsVisibleToTeam(ETeam eCheckTeam);

native simulated function bool IsVisible();

native function SetTeamType(ETeam eNewTeam);

native simulated function bool IsFriendly(Actor kCheckActor);

native simulated function bool IsTeamFriendly(ETeam eCheckTeam);
...
//XCOM additions
//----------------------
simulated function ServerDestroy();

simulated event OnCleanupWorld();

// Export UActor::execSubscribeToOnCleanupWorld(FFrame&, void* const)
native simulated function SubscribeToOnCleanupWorld();

// Export UActor::execUnsubscribeFromOnCleanupWorld(FFrame&, void* const)
native simulated function UnsubscribeFromOnCleanupWorld();

final function bool FindActorByName(name ActorName, out Actor out_Actor)
{
    local Actor A;
